# NotificationGroupMacro Constructor 
 

Initializes a new instance of the <a href="T_Lextm_SharpSnmpPro_Mib_NotificationGroupMacro">NotificationGroupMacro</a> class

**Namespace:**&nbsp;<a href="N_Lextm_SharpSnmpPro_Mib">Lextm.SharpSnmpPro.Mib</a><br />**Assembly:**&nbsp;SharpSnmpPro.Mib (in SharpSnmpPro.Mib.dll) Version: 2.1.2

## Syntax

**C#**<br />
``` C#
public NotificationGroupMacro(
	ISmiValue value
)
```

**VB**<br />
``` VB
Public Sub New ( 
	value As ISmiValue
)
```

**C++**<br />
``` C++
public:
NotificationGroupMacro(
	ISmiValue^ value
)
```

**F#**<br />
``` F#
new : 
        value : ISmiValue -> NotificationGroupMacro
```


#### Parameters
&nbsp;<dl><dt>value</dt><dd>Type: <a href="T_Lextm_SharpSnmpPro_Mib_ISmiValue">Lextm.SharpSnmpPro.Mib.ISmiValue</a><br /></dd></dl>

## See Also


#### Reference
<a href="T_Lextm_SharpSnmpPro_Mib_NotificationGroupMacro">NotificationGroupMacro Class</a><br /><a href="N_Lextm_SharpSnmpPro_Mib">Lextm.SharpSnmpPro.Mib Namespace</a><br />