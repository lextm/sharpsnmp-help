# OperationMacro.ArgumentIdentifier Property 
 

**Namespace:**&nbsp;<a href="N_Lextm_SharpSnmpPro_Mib">Lextm.SharpSnmpPro.Mib</a><br />**Assembly:**&nbsp;SharpSnmpPro.Mib (in SharpSnmpPro.Mib.dll) Version: 2.1.2

## Syntax

**C#**<br />
``` C#
public string ArgumentIdentifier { get; set; }
```

**VB**<br />
``` VB
Public Property ArgumentIdentifier As String
	Get
	Set
```

**C++**<br />
``` C++
public:
property String^ ArgumentIdentifier {
	String^ get ();
	void set (String^ value);
}
```

**F#**<br />
``` F#
member ArgumentIdentifier : string with get, set

```


#### Property Value
Type: <a href="https://docs.microsoft.com/dotnet/api/system.string" target="_blank" rel="noopener noreferrer">String</a>

## See Also


#### Reference
<a href="T_Lextm_SharpSnmpPro_Mib_OperationMacro">OperationMacro Class</a><br /><a href="N_Lextm_SharpSnmpPro_Mib">Lextm.SharpSnmpPro.Mib Namespace</a><br />